...
\code{covariance(object, invert)} & Extraction of the full conditional covariance matrix~$\Sigma$
                                    (\code{invert = FALSE}) or its Moore-Penrose inverse $\Sigma^+$
                                    (\code{invert = TRUE}). \\
...
for the lower triangular part of $\Sigma^+$ and its rank (degrees of freedom):
