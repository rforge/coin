...
\code{statistic(object, type, partial)}
         & Extraction of the aggregated (\code{partial = FALSE}) or partial
           ...
           (\code{type = "standardized"}) $\z$ or $\z_j$, respectively.
           For objects inheriting from \Rclass{IndependenceTestStatistic}, the
...
based on \pkg{coin} version \Sexpr{packageVersion("coin")}, available
under the terms of the General Public License from \url{https://CRAN.R-project.org/}.
\proglang{R} version \Sexpr{getRversion()} \citep{rcore2007}
