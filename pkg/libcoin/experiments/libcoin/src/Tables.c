...
/* table(ix, iy) */
void C_2dtable(int *ix, int Lx, int *iy, int Ly, int N, int *LxLy_ans)
{
                ...
                int *LxLyLb_ans) 
{
    if (LENGTH(block) == 0) {
        if ((LENGTH(weights) == 0) && (LENGTH(subset) == 0))
            C_2dtable(INTEGER(ix), C_nlevels(ix) + 1, 
                      INTEGER(iy), C_nlevels(iy) + 1, 
                      LENGTH(ix), LxLyLb_ans);
        if ((LENGTH(weights) > 0) && (LENGTH(subset) == 0))
            C_2dtable_weights(INTEGER(ix), C_nlevels(ix) + 1, 
                              INTEGER(iy), C_nlevels(iy) + 1, 
                              INTEGER(weights), LENGTH(weights), LxLyLb_ans);
        if ((LENGTH(weights) == 0) && (LENGTH(subset) > 0))
            C_2dtable_subset(INTEGER(ix), C_nlevels(ix) + 1, 
                             INTEGER(iy), C_nlevels(iy) + 1, 
                             INTEGER(subset), LENGTH(subset), LxLyLb_ans);
        if ((LENGTH(weights) > 0) && (LENGTH(subset) > 0))
            C_2dtable_weights_subset(INTEGER(ix), C_nlevels(ix) + 1, 
                                     INTEGER(iy), C_nlevels(iy) + 1, 
                                     INTEGER(weights), INTEGER(subset), 
                                     LENGTH(subset), LxLyLb_ans);
    } else {
        if ((LENGTH(weights) == 0) && (LENGTH(subset) == 0))
            C_2dtable_block(INTEGER(ix), C_nlevels(ix) + 1, 
                            INTEGER(iy), C_nlevels(iy) + 1, 
                            INTEGER(block), C_nlevels(block), LENGTH(block), 
                            LxLyLb_ans);
        if ((LENGTH(weights) > 0) && (LENGTH(subset) == 0))
            C_2dtable_weights_block(INTEGER(ix), C_nlevels(ix) + 1, 
                                    INTEGER(iy), C_nlevels(iy) + 1, 
                                    INTEGER(weights),  
                                    INTEGER(block), C_nlevels(block), 
                                    LENGTH(weights), LxLyLb_ans);
        if ((LENGTH(weights) == 0) && (LENGTH(subset) > 0))
            C_2dtable_subset_block(INTEGER(ix), C_nlevels(ix) + 1, 
                                   INTEGER(iy), C_nlevels(iy) + 1, 
                                   INTEGER(subset), LENGTH(subset), 
                                   INTEGER(block), C_nlevels(block), 
                                   LxLyLb_ans);
        if ((LENGTH(weights) > 0) && (LENGTH(subset) > 0))
            C_2dtable_weights_subset_block(INTEGER(ix), C_nlevels(ix) + 1, 
                                           INTEGER(iy), C_nlevels(iy) + 1, 
                                           INTEGER(weights), INTEGER(subset),
                                           LENGTH(subset), INTEGER(block), 
                                           C_nlevels(block), LxLyLb_ans);
    }
}
