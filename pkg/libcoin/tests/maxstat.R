...
(tst <- doTest(lev, type = "max"))
ux[tst$index]
(tst <- doTest(lev, type = "quad"))
ux[tst$index]
...
(mt <- maxstat_test(y ~ x , distrib = approximate(B = 1000)))
lev <- LinStatExpCov(ix, y, B = 1000)
(tst <- doTest(lev, type = "max", ordered = FALSE))
(tst <- doTest(lev, type = "quad", ordered = FALSE))
lev <- LinStatExpCov(ix, y, B = 1000, varonly = TRUE)
(tst <- doTest(lev, type = "max", ordered = FALSE))
(tst <- doTest(lev, type = "quad", ordered = FALSE))
...
(tst <- doTest(lev, type = "quad", ordered = FALSE))
lev <- LinStatExpCov(ix, y, block = blk, B = 1000, varonly = TRUE)
try(tst <- doTest(lev, type = "max", ordered = FALSE))
