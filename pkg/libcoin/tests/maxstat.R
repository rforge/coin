...
(tst <- Test(lev, type = "max"))
ux[tst$index]
(tst <- Test(lev, type = "quad"))
ux[tst$index]
...
(mt <- maxstat_test(y ~ x , distrib = approximate(B = 1000)))
lev <- LinStatExpCov(ix, y, B = 1000)
(tst <- Test(lev, type = "max", ordered = FALSE))
(tst <- Test(lev, type = "quad", ordered = FALSE))
lev <- LinStatExpCov(ix, y, B = 1000, varonly = TRUE)
(tst <- Test(lev, type = "max", ordered = FALSE))
(tst <- Test(lev, type = "quad", ordered = FALSE))
...
(tst <- Test(lev, type = "quad", ordered = FALSE))
lev <- LinStatExpCov(ix, y, block = blk, B = 1000, varonly = TRUE)
try(tst <- Test(lev, type = "max", ordered = FALSE))
