...
set.seed(290875)
ct_m <- lc("chisq_test",as.table(tea),
...
set.seed(290875)
it_m_s <- lc("independence_test",as.table(tea),
...
set.seed(290875)
it_m_q <- lc("independence_test",as.table(tea),
                            distribution = approximate(B = 10000),
                            teststat = "quad")
...
set.seed(290875)
it_m_s_gr <- lc("independence_test",as.table(tea),
                               distribution = approximate(B = 10000),
                               teststat = "scalar",
                               alternative = "greater")
(p <- pvalue(it_m_s_gr))
...
ct_e <- lc("chisq_test",as.table(davis),
                   distribution = "exact")
stopifnot(isequal(round(pvalue(ct_e), 4), 0.2860))
stopifnot(isequal(round(midpvalue(ct_e), 4), 0.1527))

it_e_s <- lc("independence_test",as.table(davis),
                            distribution = "exact",
                            teststat = "scalar")
stopifnot(isequal(pvalue(it_e_s), pvalue(ct_e)))
stopifnot(isequal(midpvalue(it_e_s), midpvalue(ct_e)))

it_e_q <- lc("independence_test",as.table(davis),
                            distribution = "exact",
                            teststat = "quad")
stopifnot(isequal(pvalue(it_e_q), pvalue(ct_e)))
stopifnot(isequal(midpvalue(it_e_q), midpvalue(ct_e)))
...
### Additional results: Monte Carlo
set.seed(290875)
ct_m <- lc("chisq_test",as.table(davis),
                   distribution = "approximate")
(p <- pvalue(ct_m))
pci <- attr(p, "conf.int")
...
set.seed(290875)
it_m_s <- lc("independence_test",as.table(davis),
                            distribution = approximate(B = 10000),
                            teststat = "scalar")
stopifnot(isequal(pvalue(it_m_s), pvalue(ct_m)))
stopifnot(isequal(midpvalue(it_m_s), midpvalue(ct_m)))
...
set.seed(290875)
it_m_q <- lc("independence_test",as.table(davis),
                            distribution = approximate(B = 10000),
                            teststat = "quad")
stopifnot(isequal(pvalue(it_m_q), pvalue(ct_m)))
stopifnot(isequal(midpvalue(it_m_q), midpvalue(ct_m)))
...
ct_e <- lc("chisq_test",as.table(cardiac),
                   distribution = "exact")
stopifnot(isequal(round(pvalue(ct_e), 4), 0.0544))
stopifnot(isequal(round(midpvalue(ct_e), 4), 0.0297))

it_e_s <- lc("independence_test",as.table(cardiac),
                            distribution = "exact",
                            teststat = "scalar")
stopifnot(isequal(pvalue(it_e_s), pvalue(ct_e)))
stopifnot(isequal(midpvalue(it_e_s), midpvalue(ct_e)))

it_e_q <- lc("independence_test",as.table(cardiac),
                            distribution = "exact",
                            teststat = "quad")
stopifnot(isequal(pvalue(it_e_q), pvalue(ct_e)))
stopifnot(isequal(midpvalue(it_e_q), midpvalue(ct_e)))
...
### Additional results: Monte Carlo
set.seed(290875)
ct_m <- lc("chisq_test",as.table(cardiac),
                   distribution = approximate(B = 10000))
(p <- pvalue(ct_m))
pci <- attr(p, "conf.int")
...
set.seed(290875)
it_m_s <- lc("independence_test",as.table(cardiac),
                            distribution = approximate(B = 10000),
                            teststat = "scalar")
stopifnot(isequal(pvalue(it_m_s), pvalue(ct_m)))
stopifnot(isequal(midpvalue(it_m_s), midpvalue(ct_m)))
...
set.seed(290875)
it_m_q <- lc("independence_test",as.table(cardiac),
                            distribution = approximate(B = 10000),
                            teststat = "quad")
stopifnot(isequal(pvalue(it_m_q), pvalue(ct_m)))
stopifnot(isequal(midpvalue(it_m_q), midpvalue(ct_m)))
...
ct_e <- lc("chisq_test",as.table(exfoliative),
                   distribution = "exact")
stopifnot(isequal(round(pvalue(ct_e), 4), 0.0629))
stopifnot(isequal(round(midpvalue(ct_e), 4), 0.0447))

it_e_s <- lc("independence_test",as.table(exfoliative),
                            distribution = "exact",
                            teststat = "scalar")
stopifnot(isequal(pvalue(it_e_s), pvalue(ct_e)))
stopifnot(isequal(midpvalue(it_e_s), midpvalue(ct_e)))

it_e_q <- lc("independence_test",as.table(exfoliative),
                            distribution = "exact",
                            teststat = "quad")
stopifnot(isequal(pvalue(it_e_q), pvalue(ct_e)))
stopifnot(isequal(midpvalue(it_e_q), midpvalue(ct_e)))
...
### Additional results: Monte Carlo
set.seed(290875)
ct_m <- lc("chisq_test",as.table(exfoliative),
                   distribution = approximate(B = 10000))
(p <- pvalue(ct_m))
pci <- attr(p, "conf.int")
...
set.seed(290875)
it_m_s <- lc("independence_test",as.table(exfoliative),
                            distribution = approximate(B = 10000),
                            teststat = "scalar")
stopifnot(isequal(pvalue(it_m_s), pvalue(ct_m)))
stopifnot(isequal(midpvalue(it_m_s), midpvalue(ct_m)))
...
set.seed(290875)
it_m_q <- lc("independence_test",as.table(exfoliative),
                            distribution = approximate(B = 10000),
                            teststat = "quad")
stopifnot(isequal(pvalue(it_m_q), pvalue(ct_m)))
stopifnot(isequal(midpvalue(it_m_q), midpvalue(ct_m)))
...
set.seed(290875)
st_m_s <- lc("symmetry_test",as.table(ahr),
                        distribution = approximate(B = 10000),
                        teststat = "scalar")
...
set.seed(290875)
st_m_q <- lc("symmetry_test",as.table(ahr),
                        distribution = approximate(B = 10000),
                        teststat = "quad")
...
### Results from Fagerland, Lydersen and Laake (2013, p. 7, Tab. 6)
mt_e <- lc("mh_test",as.table(therapy),
                distribution = "exact")
stopifnot(isequal(round(pvalue(mt_e), 4), 0.0525))
stopifnot(isequal(round(midpvalue(mt_e), 4), 0.0347))

st_e_s <- lc("symmetry_test",as.table(therapy),
                        distribution = "exact",
                        teststat = "scalar")
stopifnot(isequal(pvalue(st_e_s), pvalue(mt_e)))
stopifnot(isequal(midpvalue(st_e_s), midpvalue(mt_e)))

st_e_q <- lc("symmetry_test",as.table(therapy),
                        distribution = "exact",
                        teststat = "quad")
stopifnot(isequal(pvalue(st_e_q), pvalue(mt_e)))
stopifnot(isequal(midpvalue(st_e_q), midpvalue(mt_e)))
...
### Additional results: Monte Carlo
set.seed(290875)
mt_m <- lc("mh_test",as.table(therapy),
                distribution = approximate(B = 10000))
(p <- pvalue(mt_m))
pci <- attr(p, "conf.int")
...
set.seed(290875)
st_m_s <- lc("symmetry_test",as.table(therapy),
                        distribution = approximate(B = 10000),
                        teststat = "scalar")
stopifnot(isequal(pvalue(st_m_s), pvalue(mt_m)))
stopifnot(isequal(midpvalue(st_m_s), midpvalue(mt_m)))
...
set.seed(290875)
st_m_q <- lc("symmetry_test",as.table(therapy),
                        distribution = approximate(B = 10000),
                        teststat = "quad")
stopifnot(isequal(pvalue(st_m_q), pvalue(mt_m)))
stopifnot(isequal(midpvalue(st_m_q), midpvalue(mt_m)))
