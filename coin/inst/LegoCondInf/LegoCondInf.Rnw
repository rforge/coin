...
block factor with a $c_{quad}$-type test statistic, i.e. the conditional
...
it_alz
@
suggests that there is some deviation from independence. 
By default, the influence function $g$ and the transformation $h$ 
are dummy codings of the smoking behavior and the disease status and
the linear multivariate statistic $\T$ is the contincency table of 
both variables 
<<alzheimer-statistic, echo = TRUE>>=
statistic(it_alz, "linear")
margin.table(alzheimer, 1:2)
@
...
<<alzheimer-statistic, echo = TRUE, eval = FALSE>>=
covariance(it_alz)
@
<<alzheimer-statistic, echo = FALSE>>=
round(covariance(it_alz), 3)
@
Here, the 
conditional distribution was approximated by its limiting distribution
which leads to the same $p$-value as the unconditional test
...
@
leads to the impression that patients suffering from Alzheimer's disease
...
@
Alternatively and more conveniently, we want to switch to the $p$-value scale. 
Here, we choose step-down
...
the internals used to discretize the smoking measurements and we can setup
a linear-by-linear association test with $c_{max}$ type test statistic via
