...
null hypothesis clearly depends on the unkown distribution of the data and
...
that the null distribution can be derived analytically. Conditional tests
...
refrain from stepping into this mostly philosphical debate, noting that most
...
trackable. The classical procedures, such as a permuation $t$ test, are part
...
inference problems or wants to perform a test not supported by the prefered
software package.
...
To fix ideas we assume that we are provided with $n$ observations
\begin{eqnarray*}
...
to those measurements a factor $b$ coding blocks may be available.
...
Ignoring the ordinal structure of the smoking behaviour, the null hypothesis
...
By default, the influence function $g$ and the transformation $h$ 
are dummy codings of the smoking behavior and the disease status
, i.e. $h(\Y_i, (\Y_1, \dots,
\Y_n)) = (1, 0, 0)$ and $g(\X_i) = (1, 0, 0 ,0)$ for a non-smoking Alzheimer
patient. Consequently, 
the linear multivariate statistic $\T$ based in $g$ and $h$ 
is the contincency table of both variables 
<<alzheimer-statistic, echo = TRUE>>=
statistic(it_alz, "linear")
@
with conditional expectation \Rcmd{expectation(it\_alz)} and
conditional covariance \Rcmd{covariance(it\_alz)}. Here, the 
conditional distribution was approximated by its limiting $\chi^2$ 
distribution which leads to exactly 
the same $p$-value as the unconditional test
...
null distribution (two-sided test)
...
pvalue(it_alzMC)
@
(the normal approximation seems is rather accurate)
...
<<alzheimer-orderedp, echo = TRUE>>=
pvalue(it_alzL, method = "single-step")
@
supports the conclusion that smoking is associated with other dementia and, 
...
number of tumors \citep[][analyse the detection time
...
it_ph 
@
Here, the influence function $h$ computes the logrank scores of the survival
time and time to first tumor and the number of tumors, i.e. for the 
...
@
It is important to note that the global test utilizes the complete
covariance matrix 
...
directly incorporates the covariances structure and leads to a very similar 
$p$-value. 
%%%% $\Sexpr{round(pvalue(it_phq), 5)}$.

The deviations from the partial null hypotheses, i.e. the independence of
each single response and the treatments, can be inspected by the standardized
linear statistic $\T$
...
functionality to implement a complete new test procedure. 
...
incomplete-symmetrical chomosome aberrations (see Figure \ref{mercurybox}). 
The observations are partially
ordered: an observation is smaller than another when all three variables
are smaller. The rank score for observation $i$ is the number of
observations what are larger (following the above criterion) 
...
par(cex.lab = 1.3, cex.axis = 1.3)
layout(matrix(1:3, ncol = 3))
boxplot(I(log(mercury)) ~ group, data = mercuryfish, ylab = "Mercury Blood Level (log scale)")
boxplot(abnormal ~ group, data = mercuryfish, 
        ylab = "Abnormal Cells (in %)")
boxplot(ccells ~ group, data = mercuryfish, 
        ylab = "Chomosome Aberrations (in %)")
@
...
(\Rcmd{qperm}) for the conditional distribution. When only a small number of
...
\begin{figure}
\begin{center}
<<mercuryfish-ppermplot, echo = FALSE, fig = TRUE, height = 5, width = 8>>=
