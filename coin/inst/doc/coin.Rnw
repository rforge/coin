...
limiting distribution. A large amount of classical test procedures for 
continuous, categorical and censored data fits into this framework, for
...
\texttt{independence\_test}. Because concenience functions for the most
prominent problems are available, users will not have to use this
extremely flexible procedure. Currently, the following test procedures are
available: 
\begin{center}
\begin{tabular}{ll}
...
\texttt{mh\_test} &  marginal homogenity test. \\
\end{tabular}
\end{center}
...
\in \R^{pq}.
\end{eqnarray}
Here, $g: \X \rightarrow \R^{p}$ is a transformation of
the $\X$ measurements and the \emph{influence function}
$h: \Y \times \Y^n \rightarrow
...
\texttt{y ~ x | b}, weights can be defined by a formula with one variable on
...
The test procedures implemented in package `coin' are continuously checked
...
exact, approximativ and asymptotic $p$-values for data examples given in the 
\texttt{StatXact-6} 
user manual \cite{StatXact6} are compared with the results reported in the
