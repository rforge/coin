...
are stored, the whole covariance matrix can be computed upon request (see below).
...
A univariate test statistic $c$ can be stored in slot \code{teststatistic}. 
...
standardized contingency tables. For more practical examples, including
