...
For our object of class \Rclass{IndependenceLinearStatistic} we can now
...
\code{statistic(object, type)} & Extraction of the linear statistic $\t$ (\code{type = "linear"}), 
		     the standardized statistic $\z$ (\code{type = "standardized"})
  		     or the final test statistic $c$ (\code{type = "test"}, only for \Rclass{IndependenceTestStatistic} objects). \\
...
Three types of univariate test statistics are implemented in \pkg{coin}
and explained in the following: scalar test statistics
$c_\text{scalar}$, maximum-type statistics $c_\text{max}$ and quadratic
forms $c_\text{quad}$.
...
safisfaction in four (ordinal) classifications.  It seems natural to
...
\end{center}
\end{figure}
Here, we focus on conditional tests for independence of income and job satisfaction. 
Ignoring the ordering of the levels in both variables, the conditional Cochran-Mantel-Haenszel test is
...
@
The standardized contingency table can now be interpreted in the same way as 
Pearson residuals. Althought a $p$ value of $\Sexpr{round(pvalue(it), 3)}$
doesn't lead to a rejection of the independence hypothesis, the positive diagonal 
and negative off-diagonal elements suggest that a test statistic taking the 
ordering of the levels into account may very well detect a deviation from independence.
...
\cite{agresti2002} used equidistant scores for \code{Income}, one could also 
assign mid-points of income intervals to the \code{Income} scores---see \cite{Hothorn:2006:AmStat}
for an application of non-equidistant scores.
...
for norminal variables (which is, however, not the case in this particular application).
...
standardized contingency tables. For more practical examples, including applications with numeric variables, 
