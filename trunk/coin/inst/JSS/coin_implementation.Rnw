...
\end{eqnarray*}
The conditinal expectation of the linear statistic $\T_j$ in block $j$ is 
\begin{eqnarray}
...
& = &
    \frac{w_{\cdot j}}{w_{\cdot j} - 1}  \V(h | S_j) \otimes
        \left(\sum_i I(b_i = j) w_i  g(\X_i) \otimes I(b_i = j) w_i  g(\X_i)^\top \right)
\label{expectcovar}
\\
...
\begin{eqnarray*}
c_\text{scalar}(\T, \mu, \Sigma) = (\t - \mu) \times \Sigma^{-1/2}.
\end{eqnarray*}
In the multivariate case ($pq > 1$), the choice of the test statistic depends
on the alternative we have in mind:
\begin{eqnarray*}
...
the \pkg{coin} package is \begin{eqnarray*}
...
distribution (or an approximation therof). In addition, 
...
exact contitional null distribution are implemented in \proglang{C} as well.
(In addition,
...
this choice is due to the lazyness of the authors, however, it was our aim
to mimic the user interfaces of the classical tests, such as \Rcmd{wilcox.test},
as closely as possible.

\subsection{General Independence Tests}

The generic function \Rcmd{independence\_test} has methods for tables (both
$\X$ and $\Y$ being univariate factors), formulae of the form
\begin{Sinput}
y ~ x | block
\end{Sinput}
or
...
\Rarg{xtrafo} referes to the transformation $g$ and \Rarg{ytrafo} to the 
...
null distribution to be used as reference distribution can be choosen either by a
...
\Rcmd{scalar} \\
Median Test & \Rcmd{median\_test} & \Rcmd{f\_trafo} & \Rcmd{median\_trafo} &
\Rcmd{scalar} \\
Ansari-Bradley & \Rcmd{ansari\_test} & \Rcmd{f\_trafo} & \Rcmd{ansari\_trafo} &
\Rcmd{scalar} \\
Log Rank Test & \Rcmd{surv\_test} & \Rcmd{f\_trafo} & \Rcmd{logrank\_trafo} &
\Rcmd{quad} \\
Kruskal-Wallis & \Rcmd{kruskal\_test} & \Rcmd{f\_trafo} & \Rcmd{rank} &
\Rcmd{quad} \\
Fligner Test & \Rcmd{fligner\_test} & \Rcmd{f\_trafo} & \Rcmd{fligner\_trafo} &
\Rcmd{quad} \\
Spearman Test & \Rcmd{spearman\_test} & \Rcmd{rank} & \Rcmd{rank} &
\Rcmd{scalar} \\
Cochran-Mantel-Haenzsel & \Rcmd{cmh\_test} & \Rcmd{f\_trafo} & \Rcmd{f\_trafo} &
\Rcmd{quad} \\
Pearson's $\chi^2$ Test & \Rcmd{chisq\_test} & \Rcmd{f\_trafo} & \Rcmd{f\_trafo} &
\Rcmd{quad} \\
Cochran-Armitage / Linear Association & \Rcmd{lbl\_test} & Scores & any &
\Rcmd{scalar} \\
Permutation test & \Rcmd{oneway\_test} & \Rcmd{f\_trafo} & any & any \\
\multicolumn{5}{c}{Dependent Samples} \\
Test & Function & \Rcmd{xtrafo} & \Rcmd{ytrafo} & \Rcmd{teststat} \\ \hline
Friedman Test & \Rcmd{friedman\_test} & \Rcmd{f\_trafo} & \Rcmd{rank} &
\Rcmd{quad} \\
...
\Rcmd{scalar} \\
\end{tabular}
\caption{Convenience Functions \label{confct}}
\end{center}
\end{sidewaystable}
...
@
and standardized contingency table
<<jobsatisfaction-stat, echo = TRUE>>=
statistic(cmh_test(jobsatisfaction), "standardized")
@
A new test, based on the maximum of the standardized contingency
table is easily defined:
...
immidiately available. Taking the ordinal scale level of both
variables into account, a linear by linear association test
is easily obtainable via
