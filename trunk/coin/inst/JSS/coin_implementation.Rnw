...
challenging because of the many ties in the response ($\Sexpr{sum(rotarod$time == 300)}$
...
two-sided exact $p$~value for any 2-sample contrast (based on some strictly monotonic
transformation of the response) very simple: $p = 2 \cdot {19 \choose 12} / {24 \choose 12}
...
\begin{description}
\item[\code{statistic}:] extracts the linear statistic $\t$, 
                           the standardized statistic $\z$ or the final test statistic.
\item[\code{expectation}:] extracts the conditional expectation $\mu$.
\item[\code{covariance}:] extracts the complete conditional covariance matrix 
                            $\Sigma$.
\item[\code{variance}:] extracts the diagonal elements of the conditional covariance matrix
                          $\text{diag}(\Sigma)$.
\end{description}
...
variance(its)
@
based upon which we can set up a test statistics. 
...
\begin{description}
\item[\code{pvalue}:] computes the $p$~value (plus a confidence interval if
 Monte-Carlo procedures have been used) based on an observed test statistic $c$ and its
 conditional null distribution.
\item[\code{pperm}:] evaluates the cumulative distribution function.
\item[\code{dperm}:] evaluates the probability density function. 
\item[\code{qperm}:] evaluates the quantile function.
\item[\code{support}:] returns the support of the null distribution. %Z% in which form?
\end{description}
...
%%In addition, \code{show} methods
%%are defined for classes \Rclass{ScalarIndependenceTest}, \Rclass{MaxTypeIndependenceTest}
%%and \Rclass{QuadTypeIndependenceTest}, converting these \proglang{S}4 
%%objects to an informal \proglang{S3} object of class \Rclass{htest} for which
%%a \code{print} method is available that most \proglang{R} users are familiar with.
...
%%A \code{confint} method is available for these special cases.
...
Function \code{independence_test} performs conditional independence tests
utilizing the building blocks introduced in the previous section. It is
a generic with methods for 
class \Rclass{IndependenceProblem} as well as \Rclass{formula}
and \Rclass{table} objects providing the user with the same type of interfaces
they are used to from base \proglang{R}.

For the \Rclass{formula} interface of 
\code{independence_test}, the left hand side
...
Two- and three-dimensional tables (in case both $\X$ and $\Y$ are univariate factors) can be supplied to the \Rclass{table} method of \linebreak
\code{independence_test}. The third variable is then interpreted as block.

The main workhorse is the method for class \Rclass{IndependenceProblem} which is defined as 
\begin{Sinput}
...
influence function $h$ (function \code{trafo} implements reasonable defaults, 
...
character string or by functions \code{exact}, \code{approximate} and \code{asymptotic}
...
possible permutations of the observations: The function \code{sexact}
implements a function which computes the exact conditional distribution 
function which is then passed to \code{independence_test} via its 
...
sexact <- function(object) {
  ...
  p <- function(q) 1 - mean(pstats > q) ## FIXME: can this be correct?
  new("PValue", p = p, pvalue = p)
}
@
In \code{independence_test}, this function is used to compute the distribution function and $p$~value:
<<Ex-distribution, echo = TRUE>>=
...
\code{independence_test} for both $g$ and $h$ in case of numeric
variables $\X$ and $\Y$, respectively (function \code{id_trafo}). Factors
are transformed to indicator variables (function \code{f_trafo})
and censored variables are transformed to log rank scores (function \code{logrank_trafo}):
\begin{Sinput}
...
specified as arguments to \code{trafo}.
...
\section{Permutation tests in practice} \label{sec:practice}
...
of the Mood test against scale alternatives, i.e., using tranformation
$h(Y_i) = (\text{rank}(Y_i) - (n + 1) / 2)^2)$,
...
<<Ex-mood, echo = TRUE>>=
independence_test(time ~ group, data = rotarod, distribution = exact(),
  ytrafo = function(y) (rank(y) - (nrow(y) + 1) / 2)^2)
@
