...
\proglang{S} language or the tools implemented in \proglang{StatXact}, \proglang{LogXact},
\proglang{Stata}, and \proglang{Testimate}
...
conditional expectation and variance (\ref{standstat}). 
...
The \pkg{coin} package imports packages \pkg{mvtnorm} \cite{PKG:mvtnorm} 
for the
evaluation of the multivariate normal distribution and package \pkg{modeltools}
\cite{PKG:modeltools} for formula parsing.
...
given in the \proglang{StatXact}~6 user manual \citep{StatXact6} are compared
