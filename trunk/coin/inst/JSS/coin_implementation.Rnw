...
as well as multivariate independence problems can embedded into a common theory.
...
be set-up easily using the tools described here.
...
implemented independently, maybe
with the exception of conditional versions of linear rank statistics.

Novel theoretical insights by \cite{StrasserWeber1999} opened the way to
a unified treatment of a huge class of permutation tests. \cite{Hothorn:2006:AmStat} 
introduce a computational counterpart of the theoretical framework for
permutation tests by \cite{StrasserWeber1999}. The computation framework
is implemented in the \pkg{coin} add-on package to the \proglang{R} system
...
conditional Monte-Carlo methods or asymptotic arguments, are implemented in
...
that $\w_i$ observations with measurements $\Y_i$ and $\X_i$ are available, 
...
in $k$ blocks $k$ blocks ($b_i = 1, \dots, k$) and this only re-randomizations
...
approximate the exact conditional distribution. Each function taking an
...
This new (at least we are not aware of a name for such a procedure) test is easily defined:
...
Henric Nilsson provided clarification and examples for the Stuart-Maxwell test and for
reporting bugs. The work of T. Hothorn was supported by Deutsche Forschungsgemeinschaft (DFG) 
