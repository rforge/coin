...
toLatex.c2t <- function(object, center = TRUE, ...) {        
    ...
    ### slots
    RET <- c(RET, " & \\\\", "Slots & Type \\\\ \\hline ",
             apply(object, 1, function(x) {
                 x <- paste("\\Rcmd{", x, "}", sep = "")
                 paste(paste(paste(x, collapse = " & "), "\\\\"))
             }),
             "\\hline")
...
contains the (possibly multivariate) linear statistic standardize by its 
conditional expectation and variance. The univariate test statistic, 
two types of test statistics are given in the next section,
are stored in slot \Rarg{teststatistic}.
...
@
both defining a character vector specifying the alternative to test against (\Rcmd{two.sided},
\Rcmd{greater} and \Rcmd{less}. Quadratic forms always lead to two-sided tests but
require the Moore-Penrose inverse of the conditional covariance matrix $\Sigma^+$ and
its rank (degrees of freedom) to be known:
...
null distribution, computed for example via Streitberg/Roehmel),
\Rclass{ApproxNullDistribution} (Monte-Carlo procedures) and \Rclass{AsymptNullDistribution}
(asyptotic approximations via multivariate normal or $\chi^2$ distribution).

\subsection{Classes for Conditional Tests}

A conditional test is represented by it null distribution and
a character string giving the name of the test in class \Rclass{IndependenceTest}:
<<IndependenceTest, results = tex>>=
toLatex(c2t("IndependenceTest"))
@
However, the null distribution itself needs to be extended by a description
of the test problem this distribution is useful for. Here, the 
type of test statistic used constitutes three classes for the description of
conditional tests. Objects of class \Rclass{ScalarIndependenceTest} describe 
tests with scalar test statistics ($pq = 1$)
<<ScalarIndependenceTest, results = tex>>=
toLatex(c2t("ScalarIndependenceTest"))
@
whereas maximum-type and quadratic statistics are given by classes
<<MaxTypeIndependenceTest, results = tex>>=
toLatex(c2t("MaxTypeIndependenceTest"))
@
and
<<QuadTypeIndependenceTest, results = tex>>=
toLatex(c2t("QuadTypeIndependenceTest"))
@
...
\item[\Rcmd{expectation()}:] extracts the conditional expectation $\E(\T | S)$.
...
are available:
\begin{itemize}
\item[\Rcmd{pvalue()}:] computes the $P$-value.
...
\Rclass{IndependenceTest} as well.
...
and conditional covariate matrix $\Sigma$ are coded in \proglang{C}. (In addition,
some helper functions, e.g. the Kronecker product etc., are implemented in \proglang{C} as well.)
The complete source-code and documentation can be accessed via
...
Wilcoxon-Mann-Whitney & \Rcmd{wilcox\_test} & \Rcmd{factor\_trafo} & \Rcmd{rank} &
\Rcmd{scalar} \\
Normal Quantiles & \Rcmd{normal\_test} & \Rcmd{factor\_trafo} & \Rcmd{normal\_trafo} &
\Rcmd{scalar} \\
Median Test & \Rcmd{median\_test} & \Rcmd{factor\_trafo} & \Rcmd{median\_trafo} &
\Rcmd{scalar} \\
Ansari-Bradley & \Rcmd{ansari\_test} & \Rcmd{factor\_trafo} & \Rcmd{ansari\_trafo} &
\Rcmd{scalar} \\
Log Rank Test & \Rcmd{surv\_test} & \Rcmd{factor\_trafo} & \Rcmd{logrank\_trafo} &
\Rcmd{quad} \\
Kruskal-Wallis & \Rcmd{kruskal\_test} & \Rcmd{factor\_trafo} & \Rcmd{rank} &
\Rcmd{quad} \\
Fligner Test & \Rcmd{fligner\_test} & \Rcmd{factor\_trafo} & \Rcmd{fligner\_trafo} &
\Rcmd{quad} \\
Spearman Test & \Rcmd{spearman\_test} & \Rcmd{rank} & \Rcmd{rank} &
\Rcmd{scalar} \\
Cochran-Mantel-Haenzsel & \Rcmd{cmh\_test} & \Rcmd{factor\_trafo} & \Rcmd{factor\_trafo} &
\Rcmd{quad} \\
Pearson's $\chi^2$ Test & \Rcmd{chisq\_test} & \Rcmd{factor\_trafo} & \Rcmd{factor\_trafo} &
\Rcmd{quad} \\
Cochran-Armitage / Linear Association & \Rcmd{lbl\_test} & Scores & any &
\Rcmd{scalar} \\
Permutation test & \Rcmd{oneway\_test} & \Rcmd{factor\_trafo} & any & any \\
\multicolumn{5}{c}{Dependent Samples} \\
Test & Function & \Rcmd{xtrafo} & \Rcmd{ytrafo} & \Rcmd{teststat} \\ \hline
Friedman Test & \Rcmd{friedman\_test} & \Rcmd{factor\_trafo} & \Rcmd{rank} &
\Rcmd{quad} \\
Maxwell-Stuart & \Rcmd{mh\_test} & \Rcmd{factor\_trafo} & \Rcmd{factor\_trafo} &
\Rcmd{quad} \\
Wilcoxon Signed Rank & \Rcmd{wilcoxsign\_test} & \Rcmd{factor\_trafo} & \Rcmd{rank} &
\Rcmd{scalar} \\
\end{tabular}
