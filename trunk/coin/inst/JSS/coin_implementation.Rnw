...
%%are offered as well.
%% Beispiel, vielleicht
%% independence_test(time ~ group, data = rotarod, ytrafo = rank, distribution = exact())

Before dealing with the technical details, consider the following motivating example.
...
if the response is independent of the group assignment. The theoretical
ingredients of the \cite{StrasserWeber1999} framework and their representation
in a class structure is described in Section~\ref{sec:theory}.

The hypothesis of independence of length of time and group assigment is
described by a formula which, together with a data frame \code{rotarod}, 
is specified as argument of \Rcmd{independence_test} as follows
...
it relys on the same print methods as the classical tests do, more details
on the user interface are given in Section~\ref{sec:ui}

Based on a normal approximation of the permutation distribution, the hypothesis
can be rejected at level $\alpha = 0.05$. However, given the small sample
size, it is worth the effort to compute the exact permutation distribution
by changing the \code{distribution} argument
<<motivation-perm-exact, echo = TRUE>>=
pvalue(independence_test(time ~ group, 
                         data = rotarod, distribution = exact()))
@
Other methods for extracting information from the output
of \Rcmd{independence_test}, such as the test statistic or its conditional 
variance, are described in Section~\ref{sec:methods}.
...
our case, for transforming the response variable:
...
as will be discussed in Sections~\ref{sec:theory} and \ref{sec:ui}.


\section{Theory and classes} \label{sec:theory}
