...
\Abstract{
  ...
  conceptual framework that embeds the underlying test procedures into a common
  ...
  functions for important special cases can be set up easily. Specialized
  convenience functions are available for location and scale problems in
  two or more sample, independence in two- or three-way contingency tables,
...
\Address{
  ...
  URL: \url{http://www.imbe.med.uni-erlangen.de/~hothorn/}\\
...
independence hypotheses is a very old, yet very powerful, %Z% this sounds somewhat weak
idea \citep{fisher1935,Ernst2004}. Conditional
inference procedures, or simply \textit{permutation} or \textit{re-randomization}
tests, are rather popular %Z% this is definitely to weak
and, consequently, are implemented in 
many different statistical computing environments. These implementations %Z% examples?
...
underlying theory. For convenience, a set of wrapper 
...
upon which the package is based. Formal \proglang{S4} classes defining data objects, objects describing
...
arbitrary alternatives. %Z% expand on this so that later on it is clear what
                        %Z% one- or two-sided alternatives are.
...
%Z% The role of g() and h() has to be illustrated at least briefly
%Z% to give the readers a chance to understand what is going on.
%Z% (The paper should be self-contained.)
...
%Z% I guess this could be re-structured, first giving the general approach (with implementation)
%Z% and then the three special cases. Currently, the mixing is somewhat confusing.
...
(\ref{linstat}) by
\begin{eqnarray*}
\frac{\mathbf{t} - \mu}{\text{diag}(\Sigma)^{1/2}}. %Z% we really need a symbol for this
                                                    %Z% maybe \z? :-)
\end{eqnarray*}
...
%Z% Without further explanation the alternative discussion is a bit cryptic.
...
conditional expectation and variance. A univariate test statistic $c$ is
stored in slot \code{teststatistic}. The \code{estimates} slot
may contain parameter estimates where available. %Z% This is rather vague...
...
can be computed directly in the univariate case ($pq = 1$) %Z% shouldn't we use the c_scalar notation now?
...
\Rclass{IndependenceTest} as well. %Z% ...simply extracting the corresponding quantity
                                   %Z% from the contained \Rclass{IndependenceTestStatistic}. ?
...
\Rcmd{wilcox.test}, as closely as possible. The workhorse is the
\Rcmd{independence_test} method for \Rclass{IndependenceProblem} objects.
But for convenience additional wrapper functions for certain special
cases are available along with corresponding \Rclass{formula}
and \Rclass{table} methods to provide user with the same type of interfaces
...
%Z% Maybe cor_test() as an analogon to cor.test()?
%Z% Or maybe change spearman_test() to cor_test() with a similar interface
%Z% as cor.test and also report sample estimates?
...
\multicolumn{5}{c}{Independent Samples} \\
\multicolumn{5}{c}{} \\
Wilcoxon-Mann-Whitney & \Rcmd{wilcox_test} & \Rcmd{f_trafo} & \Rcmd{rank} &
\code{"scalar"} \\
Normal quantiles & \Rcmd{normal_test} & \Rcmd{f_trafo} & \Rcmd{normal_trafo} &
\code{"scalar"} \\
Median & \Rcmd{median_test} & \Rcmd{f_trafo} & \Rcmd{median_trafo} &
\code{"scalar"} \\
Ansari-Bradley & \Rcmd{ansari_test} & \Rcmd{f_trafo} & \Rcmd{ansari_trafo} &
\code{"scalar"} \\
Log rank & \Rcmd{surv_test} & \Rcmd{f_trafo} & \Rcmd{logrank_trafo} &
\code{"quad"} \\
Kruskal-Wallis & \Rcmd{kruskal_test} & \Rcmd{f_trafo} & \Rcmd{rank} &
\code{"quad"} \\
Fligner & \Rcmd{fligner_test} & \Rcmd{f_trafo} & \Rcmd{fligner_trafo} &
\code{"quad"} \\
Spearman & \Rcmd{spearman_test} & \Rcmd{rank} & \Rcmd{rank} &
\code{"scalar"} \\
Cochran-Mantel-Haenszel & \Rcmd{cmh_test} & \Rcmd{f_trafo} & \Rcmd{f_trafo} &
\code{"quad"} \\
Pearson's $\chi^2$ & \Rcmd{chisq_test} & \Rcmd{f_trafo} & \Rcmd{f_trafo} &
\code{"quad"} \\
Cochran-Armitage / Linear Association & \Rcmd{lbl_test} & scores & any &
\code{"scalar"} \\
$K$-sample permutation test & \Rcmd{oneway_test} & \Rcmd{f_trafo} & any & any \\
Maximally Selected Statistics & \Rcmd{maxstat_test} & \Rcmd{maxstat_trafo} & any & \code{"max"} \\
\multicolumn{5}{c}{} \\
\multicolumn{5}{c}{Dependent Samples} \\
\multicolumn{5}{c}{} \\
Friedman & \Rcmd{friedman_test} & \Rcmd{f_trafo} & \Rcmd{rank} &
\code{"quad"} \\
Maxwell-Stuart & \Rcmd{mh_test} & \Rcmd{f_trafo} & \Rcmd{f_trafo} &
\code{"quad"} \\
Wilcoxon signed rank & \Rcmd{wilcoxsign_test} & \Rcmd{f_trafo} & \Rcmd{rank} &
\code{"scalar"} \\ \hline
\end{tabular}
...
for computing the exact null distribution in univariate 2-sample problems
...
%Z% I wouldn't know a name either, but the test is definitely not new...
%Z% Simply omit the remarks pertaining to novelty.
is set up  easily:
