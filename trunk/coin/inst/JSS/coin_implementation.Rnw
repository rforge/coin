...
\Abstract{ The \pkg{coin} add-on package for \proglang{R} implements a
  ...
  independence problems can be embedded into a common theory.  We
  propose an \proglang{S4} class structure and generic functions
  ...
  functionality described here.  In this paper, we focus on the technical
  aspects of the transition of a theoretical concept into computational
  tools.  }
...
the \pkg{coin} \citep{pkg:coin} add-on package to the \proglang{R}
