...
of a univariate test statistic $c$ (see below)
...
A univariate test statistic $c$ is stored in slot \code{teststatistic}. 
...
@
Note that the test statistic itself is computed later on by initializing
one of the three classes extending \Rclass{"IndependenceTestStatistic"}.
...
\fixme{Could this be improved? Schon: halt immer neu das IndependenceTestProblem aufsetzen, aber
das dauert ca. 100 Jahre. Der Kommentar verwirrt nur.}
...
The job satisfaction of black males in the U.S. \citep[][Table 7.8]{agresti2002} 
is described by measures of income and job safisfaction in four (ordinal) classifications. One might postulate that as income
increases, reported job satisfaction increases as well, and vice versa.
The data (see Figure~\ref{jsplot}) is given as a three-dimensional \Rclass{table} with variables \code{Income} and
...
The scores for \code{Income} are defined by the midpoint of the income intervals.
