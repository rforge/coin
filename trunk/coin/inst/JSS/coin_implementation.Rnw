...
This data set (see Table~\ref{rotarod}), with twelve observations in
...
@
Here, the conditional Wilcoxon-Mann-Whitney test was performed via a rank-transformation
of the response.
...
(via the \Rcmd{covariance} method, see below).
...
of the Mood test against scale alternatives, i.e., using tranformation $h(Y_i) = (\text{rank}(Y_i) - (n + 1) / 2)^2)$,
