...
conditional expectation and variance. A univariate test statistic, 
we high-light two important types of test statistics below,
is stored in slot \Rarg{teststatistic}. The \Rarg{estimates} slot
...
@ %
Here, we focus on conditional tests on independence of income and job
satisfaction.  The conditional Cochran-Mantel-Haenszel test is simply
based on a $\chi^2$ (quad-type) statistic derived from the contingency
table: 
...
and exact, approximative and asymptotic $p$-values for data examples
