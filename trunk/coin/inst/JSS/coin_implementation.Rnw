...
\fixme{It must be possible to execute the code without removing the NAMESPACE!!}
...
All infrastructure is conveniently interfaced in the function \code{independence_test()},
...
in order to compare test statistics and $p$~values of the Wilcoxon-Mann-Whitney
...
Although the above piece of code looks embarrassingly simple,
the underlying computations are much more sophisticated than visible at first
sight: The data are pre-processed along with their transformations, deviations
from independence are captured by a (possibly multivariate) linear statistic,
standardized by conditional expactation and variance, and aggregated to
a final test statistic. Subsequently, the requested reference distribution is
computed, from which a $p$~value is derived, and everything is wrapped into
an object that can be conveniently printed or queried for more detailed
information. After briefly reviewing the underlying theory
from \cite{StrasserWeber1999} in Section~\ref{sec:theory}, we introduce formal 
\proglang{S}4 classes and methods capturing all the outlined steps in
Section~\ref{sec:class}. Section~\ref{sec:ui} provides further information
about the high-level user interfaces and Section~\ref{sec:practice} illustrates
how to employ the software in practice. Section~\ref{sec:oddsandends} concludes
the paper with a short dicussion, some more details about the underlying theory
...
(see Section~\ref{sec:teststat}). \fixme{Subsubsections are not numbered in JSS.
  Either refer to the whole subsection or split the subsection.}
...
In the following, we describe a class structure for
representing these theoretical objects along with possible choices of
test statistics $c$ and computations or approximations of its
associated reference distribution.
...
(via the \code{covariance()} method, see below).
...
of a univariate test statistic $c$ (see Section~\ref{sec:teststat} below)
\fixme{again: subsubsections are not numbered}
which is represented by class \Rclass{IndependenceTestStatistic}
...
stored in slot \code{teststatistic}. 
\fixme{Really? It seems to be empty in the code below.}
...
\fixme{Is the type of test statistic defined implicitely or undefined?}
...
\code{statistic(object, type)} & Extraction of the linear statistic $\t$ (\code{type = "linear"}), 
		     the standardized statistic $\z$ (\code{type = "standardized"})
  		     or the final test statistic $c$ (\code{type = "test"}, the default). \\
...
\caption{\label{tab:generics} List of generic functions with
  methods for \Rclass{IndependenceTestStatistic} objects.}
\end{center}
\end{table}
...
@
This is simply the sum of the average ranks in the control groups, i.e., $180 = 12 \cdot 15$
...
variance(its)
@
based upon which we can set up a test statistic. 
...
In the multivariate case ($pq > 1$), a natural extension is to employ a maximum-type
...
\max \left( \z \right)
    \quad \text{(greater)}
\end{eqnarray*}
for the one-sided cases. Again a special class \Rclass{MaxTypeIndependenceTestStatistic}
is available for this:
<<MaxTypeIndependenceTestStatistic, results = tex>>=
toLatex(c2t("MaxTypeIndependenceTestStatistic"))
@

The definition of one- and two-sided $p$~values 
\fixme{This somehow interrupts the flow, we haven't mentioned p values anywhere up to this point.}
used for the computations in the \pkg{coin} package is 
\begin{center}
\begin{tabular}{ll}
...
quadratic forms are only applicable to two-sided hypotheses.
...
\code{qperm(object, p)} & Evaluation of the quantile function for probability \code{p}.\\
...
\end{center}
\end{table}
\fixme{How is the support represented? Especially for the asymptotic distribution\dots}
...
\section{From classes and methods to inference functions} \label{sec:ui}

\fixme{Better section title needed: The section illustrates how to set up convenience
infrastructure that is needed repeatedly, but also allowing to plug in user-defined
parts.}
...
have flexible default or can be specified by a simple character string while
...
In order to carry out all steps described in Section~\ref{sec:class} with a single
command, \pkg{coin} provides the generic function \code{independence_test()}.
The main workhorse behind it is the method for objects of class \Rclass{IndependenceProblem}:
\begin{Sinput}
...
The three types of test discussed above are hard-coded and can be specified by 
...
and the distribution are available, as discussed in the following.

\fixme{Briefly explain scores and check.}
...
is not flexile enough in some situations and hence it is also possible to
supply a function that can compute a \Rclass{PValue} object from an
\Rclass{IndependenceTestStatistic} object.

For the most important special cases, suitable function \emph{generators}
are provided in \pkg{coin}. For example, the function \code{approximate(B = 1000)}
returns a Monte Carlo function that draws \code{B} random permutations.
Similarly, \code{exact()} and \code{approximate()} return functions computing
the exact or asymptotic null distributions, respectively. Again, computational
details in the computation of the null distribution can be controlled via arguments
...
the test for the Pearson correlation is constructed. We use a tiny artificial
...
standardize it by expectation and variance, and then sets up the 
...
@
This function can then be passed to \code{independence_test()} for computing the
distribution function and $p$~value:
<<Ex-distribution, echo = TRUE>>=
independence_test(y ~ x, data = correxample, alternative = "less",
  distribution = sexact)
@
Note that the user has to assure that the same statistic is computed inside
\code{sexact()} and that the same \code{alternative} (as \code{independence_test()}
has no means of assessing whether these are in sync).
...
is employed. \fixme{Explain var trafo and block.}
...
This amounts to using tranformation $h(Y_i) = (\text{rank}(Y_i) - (n + 1) / 2)^2)$
for the response:
<<echo=TRUE>>=
...
pluggin \code{mood_score()} into \code{independence_test()}:
<<echo = TRUE>>=
...
the appropriate \code{xtrafo} and \code{ytrafo} (and \code{teststat}).
...
\fixme{One more sentence about the data and the associated test problem is really needed here.}
<<js, echo = TRUE>>=
...
pvalue(it)
@
In this case, the null hypothesis of independence of job satisfaction
and income can be rejected in favour of a linear dependency of both variables.
For more practical examples, including applications with numeric variables, 
we refer to \cite{Hothorn:2006:AmStat}.

\fixme{Explain choice of scores.}
