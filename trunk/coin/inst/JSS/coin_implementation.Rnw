...
underlying theory. We consider a small 2-sample problem based on a rotating rats 
...
compared test statistics and $p$ values of the Wilcoxon-Mann-Whitney
...
are described in Section~\ref{sec:ui} before an example illustrating conditional
independence tests for categorical data is presented.

\section{A theory for permutation tests} \label{sec:theory}
...
\end{eqnarray}
which allows for easy construction of univariate test statistics $c = c(\t, \mu,\Sigma)$ and
derivation or approximation of their conditional distribution
unter $H_0$. In the following, we describe a class structure for
representating these theoretical objects along with possible choices of
...
of variables \code{time} and \code{group} by initializing a new
...
the identify transformation is applied to \code{time} and
...
of a univariate test statistic $c$ which is 
...
represent the data, hypothesis, linear and test statistic along with
conditional expectation and covariance matrices.

A complete description of the conditional independence test on the rotating
rats is given by an object of class \Rclass{IndependenceTest} which
...
objects of class \Rclass{IndependenceTest} as well. 
...
Function \Rcmd{independence_test} is a generic with methods for 
...
an object of class \Rclass{NullDistribution}.
...
sexact <- function() {
     function(object) {
          x <- object@xtrans  
          y <- object@ytrans  
          pstats <- apply(permutations(nrow(x)), 1, function(p) x[p,] %*% y)
          pstats <- (pstats - expectation(object)) / sqrt(variance(object)) 
          p <- function(q) 
              mean(pstats < q | abs(pstats - q) < sqrt(.Machine$double.eps))
          new("PValue", p = p, pvalue = p)
     }
}
@
In \Rcmd{independence_test}, function \Rcmd{sexact} is evaluated and its result is used to
compute the distribution function and $p$ value:
<<Ex-distribution, echo = TRUE>>=
...
(i.e., a re-implementation of function \Rcmd{wilcox.exact} in \pkg{exactRankTests})
essentially requires to implement
average ranks as influence function $h$ and to compute the exact conditional
reference distribtion, for example using the shift algorithm. The following
...
new("ScalarIndependenceTest", statistic = sits,
     distribution = ExactNullDistribution(sits, algorithm = "shift"))
@
...
independence_test(time ~ group, data = rotarod, ytrafo = rank,
                   distribution = exact())
@
Very similar, performing the exact conditional Mood 2-sample test of scale 
only requires the specification of the appropriate influence function $h$:
<<mood-test, echo = TRUE>>=
independence_test(time ~ group, data = rotarod, 
...
pvalue(independence_test(js, teststat = "max"), 
        method = "single-step")
@
These $p$ values can be interpreted similar to a standardized contingency table.
