...
\noindent
KEY WORDS: Permutation tests; Independence; Approximations; Software.
\newline
...
To fix notations,
we assume that we are provided with independent and identical distributed 
observations
$(\Y_i, \X_i)$ for $i = 1, \dots, n$, possibly organized in independent blocks.
...
= h(\Y_i, (\Y_1, \dots, \Y_n))$ must depend on the responses $(\Y_1, \dots, \Y_n)$ 
...
Resampling procedures can always be used to 
...
simple methodology in typical data analysis. All analyses are
...
categorized into three groups: short ($0-4$, $n = 24$), intermediate ($6-8$,
...
In \RR{}, the function \Rcmd{independence\_test} takes a formula 
...
formulated as \verb|elevel ~ alength|. Furthermore, the
influence function $h$ is specified via the \Rcmd{ytrafo} argument. Here, we
implement a rank transformation by choosing the \Rcmd{rank} function as
\Rcmd{numeric\_trafo} argument to the \Rcmd{trafo} function which handles
all data transformations. The default influence function for numeric
responses is the identity. The transformation $g$ can be specified by
the \Rcmd{xtrafo} argument of \Rcmd{independence\_test} which, by default,
assigns the usual dummy coding to factors. Finally, we ask
for a $c_\text{quad}$-type test statistic (\Rcmd{teststat}) and obtain the
Kruskal-Wallis statistic with limiting $\chi^2$ distribution (the default) 
as follows:
...
\T_\gamma = \vec \left( \sum_{i=1}^n \gamma^\top g(\X_i)
            h(\Y_i)^\top \right).
\end{eqnarray*}
Here, the mid-points of the intervals ($0-4$, $6-8$, $10-12$) 
used to categorize the allele lengths
are a natural choice for the score vector $\gamma$ and the linear-by-linear
...
@
This $p$-value corresponds well with Figure~\ref{alpha-box}, i.e., 
...
women and men suffering from Alzheimer's disease and 
...
and the transformation $g$ (\Rcmd{xtrafo} argument)
...
occured, i.e., perform independence tests for either all female or male
subjects separately
...
contincency table can be obtained by means of the $16$-dimensional
multivariate normal distribution of the linear statistic $\T$. Most
useful, an approximation of the $95\%$ quantile of the permutation 
...
switching to $p$-values adjusted for multiple testing by an max-$\T$
...
of UVR exposure (group A: $n = 36$ with topical vehicle and 600 Robertson--Berger units 
...
\paragraph{Two Independent Samples: Contaminated Fish Consumption.}
...
observations that are larger (following the above sketched coherence criterion) 
...
boxplot(I(log(mercury)) ~ group, data = mercuryfish, 
        ylab = "Mercury Blood Level (log scale)", varwidth = TRUE)
...
Monte-Carlo procedure, or even deriving the limiting distribution of a newly
