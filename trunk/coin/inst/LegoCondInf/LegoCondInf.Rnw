...
specification of the transformations $g(\cdot)$ and $h(\cdot)$ via 
...
\Rcmd{distribution} is used:
<<alpha-kruskal, echo = TRUE>>=
independence_test(elevel ~ alength, data = alpha, ytrafo = rank, teststat = "quad")
@
which gives equivalent results as reported by \Rcmd{kruskal.test} above.
...
\Rcmd{independence\_test}. For example, 
ignoring the ordinal structure of the allele length is
...
assign numeric scores to the length categories in the transformation
$g(\cdot)$ rather than the dummy codings used above. A natural 
...
long ($\in [10, 12]$) allele lengths. In \RR{}, this is easily
implemented as the function
<<mpoints, echo = TRUE>>=
mpoints <- function(x) c(2, 7, 11)[x$alength]
@
%FIXME% preferably without $alength
which can then be passed as \Rcmd{xtrafo} to \Rcmd{independence\_test}:
...
Note that due to usage of scalar transformations $g(\cdot)$ and 
$h(\cdot)$, the $c_\text{max}$- and $c_\text{quad}$-type test statistics are
equivalent and hence \Rcmd{teststat} is not set (defaulting to \Rcmd{"max"}).
Furthermore, it should be pointed out that using such a numerical
transformation for ordinal variables relies on the same ideas as
