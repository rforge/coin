...
An extended version of this document is available as a package vignette for
reproducing the following analyses and can be accessed via
\Rcmd{vignette("LegoCondInf", package = "coin")}
directly in \RR{}. For a general introduction to the \S{} language and the \RR{}
system we refer to textbooks such as \cite{R:Venables+Ripley:2002} or
\cite{R:Everitt+Hothorn:2006}.
...
<<alpha-kruskal-ordered, echo = FALSE>>=
pval <- pvalue(independence_test(elevel ~ alength, data = alpha, ytrafo = rank, xtrafo = mpoints))
@
The resulting $p$-value is $p = \Sexpr{round(pval, 3)}$ and 
...
<<alzheimer-men, echo = FALSE>>=
females <- alzheimer$gender == "Female"
males <- alzheimer$gender == "Male"
pfemale <- pvalue(independence_test(disease ~ smoking, data = alzheimer,
       subset = females, teststat = "quad"))
pmale <- pvalue(independence_test(disease ~ smoking, data = alzheimer,
...
($p < 0.001$ for male patients and $p = \Sexpr{round(pfemale, 3)}$
...
the editor of \textit{TAS} 
for their valuable comments which lead to substantial
