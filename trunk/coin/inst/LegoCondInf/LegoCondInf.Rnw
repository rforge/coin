...
In \RR{}, the well known function \Rcmd{kruskal.test} takes a formula
...
formulated as \verb|elevel ~ alength|. In addition, the \Rcmd{data} argument
can be used to specify a \Rclass{data.frame}, i.e., an object with the usual 
rectangular representation of $n$ observations:
<<alpha-kruskal, echo = TRUE>>=
kruskal.test(elevel ~ alength, data = alpha)
@
The implementation of the classical Kruskal-Wallis test as a part of the more 
general framework using the \Rpackage{coin} package requires the definition of an 
influence function performing a ranking of the 
expression levels. Under the null hypothesis, the $c_\text{quad}$-type Kruskal-Wallis 
...
computed. 
Similar to \Rcmd{kruskal.test}, the function \Rcmd{independence\_test} takes a formula 
\verb|elevel ~ alength| and, in addition, the
influence function $h$ is specified via the \Rcmd{ytrafo} argument. Here, we
implement a rank transformation by simply choosing the \Rcmd{rank} function.
The default influence function for numeric
...
for quadratic test statistics) 
as follows:
<<alpha-kruskal, echo = TRUE>>=
independence_test(elevel ~ alength, data = alpha, ytrafo = rank,
                  teststat = "quadtype")
@
Note that the results are equivalent to the results reported by \Rcmd{kruskal.test},
...
$\gamma \in \R^{K \times 1}$, the influence function is simply the
$n$-vector of scores attached to the level of each observation.
Here, the mid-points of the intervals ($0-4$, $5-9$, $10-12$) 
used to categorize the allele lengths
are a natural choice for those scores and the linear-by-linear
association test can be performed by attaching the scores to the variable
\Robject{alength}:
<<alpha-kruskal-ordered, echo = TRUE>>=
independence_test(elevel ~ alength, data = alpha, ytrafo = rank,
                  xtrafo = function(x) c(2, 7, 11)[x$alength])
@
...
allele lengths. Note that in the above situation $c_\text{max}$- and 
$c_\text{quad}$-type test statistics are equivalent for the univariate 
linear statistic $\T$.
...
pvalue(independence_test(disease ~ smoking, data = alzheimer,
       subset = males, teststat = "quadtype"))
@
...
contingency table can be obtained from the $16$-dimensional
