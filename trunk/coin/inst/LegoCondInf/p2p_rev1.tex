...
In this sense, Fisher's test is a permutation test. However, its 
test statistic is not covered by the theoretical framework we make use of. 

Our impression is that a rather long debate on pros and cons of permutation
tests (the main contributions are cited in the introduction) led to a standoff
...
obvious. However, since the test statistic is a sum of independence linear rank 
statistics, it should by possible to derive the asymptotic and exact distribution 
\citep{wieletal1999}.
Using the original measurements, i.e., applying the 
...
The fact that the quantile is much larger than $1.96$ is simply
a consequence of the fact that we consider a $c_\text{max}$ 
statistic where the maximum is taken over $12$
normal components, i.e., the contingency table $T$. 
Such a statistic naturally has a larger quantile than that of the standard
normal and $p$-values can be adjusted by a
...
We added the required information to the problem description. 
The test effectively aggregates the results of all three measures of contamination
such that conflicting comparisons (e.g. one up, two down) are not counted. Figure 5 can
