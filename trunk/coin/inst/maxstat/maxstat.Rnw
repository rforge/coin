...
procedures. The conditional counterparts of most of the published test 
procedures are described. The conditional asymptotic distribution for a 
large class of maximally selected statistics is given. The new framework
is used to search for a high-risk group of rectal cancer patients
...
\begin{eqnarray*}
g_j(\X_i) = \indic(\X_i \in A_j); i = 1, \dots n
\end{eqnarray*}
...
<<maxstat, echo = FALSE, fig = TRUE, width = 5, height = 5>>=
...
$T$ can be computed utilizing the \texttt{maxstat\_test()} function in the
\textsf{R} add-on-package \Rpackage{coin} \citep{Hothornetal2006,PKG:coin}. The distribution
