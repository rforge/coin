...
A global linear statistic for all binary partitions is
\begin{eqnarray*}
...
variables are given in the sequel, preceeded by a description of how to
partition the covariate space for nominal, ordered and multivariate covariates.

The most important situation of an unvariate and at least ordinally measured covariate $\X$ leads
...
Many possible influence functions are available for discrete or continous covariates;
the identify transformation is the most natural choice, rank transformations are possible as well. 
For the latter, the test statistic $T$ correspond to maximally selected rank statistics 
\citep{LausenSchumacher1992}. \cite{LausenSchumacher1992} derived an approximation for the 
unconditional asymptotic distribution. When a numeric response is censored, log-rank transformations or
...
\begin{table}
\begin{center}
\caption{T and N category of $351$ rectal cancer patients treated with a preoperative 
         radiochemotherapy. \label{tab:tn}}
...
As an illustration, we want to identify a high-risk group of patients suffering from rectal 
carcinoma, treated with a neo-adjuvant radiochemotherapy regime. Here, we
search for a combination of T and N categories that can help to differentiat between two risk groups.
Survival times of $n = \Sexpr{length(tc)}$ patients from the novum arm of CAO/ARO/AIO-94 trial 
\citep{Sauer2004} are under test. The response variable is censored, so the influence function 
$h$ computes log-rank scores. From the potentially $\Sexpr{2^(nlevels(preOP$tn)-1)}$ partitions,
...
hypothesis can be rejected. All patient with N2 or N3 are under high risk; in addition, one patient 
with T4 and N1 is under high-risk as well. Figure~\ref{fig:maxstat} depicts the absolute 
standardized statistics, showing two competing classes or patitions and Kaplan-Meier 
