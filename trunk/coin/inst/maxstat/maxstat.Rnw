...
This new approximation of the asymptotic distribution based on the multivariate
normal distribution is compared with several previously suggested approximations
in Figure~\ref{approx}. It should be pointed out that the approximations differ
...
where $B^0(t)$ is a standard Brownian bridge that is scaled to zero mean and unit
variance \citep{MillerSiegmund1982,LausenSchumacher1996}. 
The correlation for $s \le t$, the correlation is
  \[ \mbox{COV}[Z^0(s), Z^0(t)] \quad = \quad \sqrt{\frac{s (1 - t)}{t (1 - s)}}, \]
and is the exactly the same as above. 
More formally, with $t_j = \lim_{n \rightarrow \infty} n^{-1} \sum_{i = 1, \dots, n} g_j(X_i)$,
the statistic $Z_j$ and $Z^0(t_j)$ are asymptotically identical in distribution.
...
in the intervals $(t_j, t_{j + 1})$ does not have to be considered. The difference between the
two approaches is negligible if all $t_{j+1} - t_j$ are sufficiently small.
Figure~\ref{approx} shows that the exact conditional distribution (approximated by
$30,000$ permutations) is most closely captured by the conditional asymptotic distribution
suggested above. Less accurate is the improved Bonferroni correction of
\citep{Worsley1982} which also uses a fixed $p$. The two approximations of
the $\sup Z^0$ distribution for increasing $p$ \citep{Jennen:1985,Hansen:1997}
are (not surprisingly) clearly below.
\begin{figure}
\begin{center}
\caption{Approximations of the distribution of a maximally selected statistic
         for $n = 100$ observations (response rank-transformed) and $p = 20$ cutpoints.
...
of all covariates simultaneously. In all three cases, only partitions that meet the sample
...
In our framework, we always utilize the conditional null distribution
...
Due to the flexibility of the generalized framework---without restrictions
regarding the measurement scales of covariate(s) and response(s)---we can
...
the identity transformation $h(Y_i) = Y_i$. Thus, each linear statistic $\T_j$ corresponds to
...
in two ordinal covariates. To reflect the ordering, it is natural to include only
