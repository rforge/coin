...
To compare these results with current practice,
...
N catgory to distinguish more severe forms of cancer; however, it uses the split
N $\le 0$ (associated with a much smaller standardized statistic of
\Sexpr{round(n0stat, digits = 2)}) while our procedure selects N $\le 1$
...
(for which the TNM staging is derived). This and other prognostic factors for
