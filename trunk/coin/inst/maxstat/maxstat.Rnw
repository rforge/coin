...
statistics can be derived on the basis of \citet[Theorem~3]{StrasserWeber1999}. 
...
%FIXME% requires specification of a parametric model or the
% "adoption" of non-parametric techniques such as permutation
% or re-sampling approaches. Here, we adopt the latter and...
...
all permutations $\sigma \in S$ of the responses are available, see
\cite{StrasserWeber1999} or \cite{Hothornetal2006}.
...
The response variable is censored, so we choose a log-rank transformation 
as influence function $h$.
%%From the potential $\Sexpr{2^(nlevels(preOP$tn)-1)}$ partitions,
$\Sexpr{length(stat)}$ possible partitions 
meet the sample size constraints. The maximum 
...
%% in the most important situations %FIXME% simply omit
by numerical integration techniques. We thereby extend
