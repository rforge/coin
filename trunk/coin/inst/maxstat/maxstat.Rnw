...
%Z% Stress "conditional"?
%Z% Or maybe "exact" or "permutation"?
...
The conditional counterparts of most of the published test procedures are
...
log-rank statistics. Beyond applications in oncology, cutpoints for habitat
factors discriminating between forest systems with low and high abundance of
certain indicator species \citep{Muller2004} are currently implemented by
the largest European forest enterprise. %Z% interesting to Biometrics?
A review of currently published scholary articles
citing one of the last statistical papers in this area 
\citep[for example][]{Mazumdar2000,HothornLausen2003} reveals many 
more applications. %Z% You mean Google Scholar "review"?
...
statistics can be derived from Theorem 3 \cite{StrasserWeber1999}. 
...
\end{eqnarray*}
%Z% Maybe it's easier if we call this $M$ or $\T_\max$, otherwise it's easy
%Z% to confuse T and \T.

The exact conditional distribution of $T$ under $H_0$ can be approximated by 
its limiting distribution. For $n \rightarrow \infty$ the
...
When the dimensionality $pq$ becomes too large,
%Z% Well, you can always use the Monte-Carlo approximation and probably one would
%Z% want to do so if $n$ is small, right?
we can approximate the exact conditional distribution of $T$ by Monte-Carlo methods 
...
For the latter, the test statistic $T$ corresponds to maximally selected rank statistics 
...
hypothesis can be rejected. All patient with N category N2 or N3 are under high risk; in addition,
...
structure is then available from the covariate matrix $\Sigma$. Moreover, the asymptotic
...
$T$ can be computed utilizing the \texttt{maxstat\_test()} function in
\textsf{R} \citep{PKG:R} in the add-on package \Rpackage{coin} \citep{Hothornetal2006,PKG:coin}.
The distribution
of $T$ can be approximated by its asymptotic distribution or my Monte-Carlo methods 
