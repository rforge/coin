...
between two variables (e.g., a single jump in a mean function) 
...
improves upon approximations for the asymptotic distribution
currently in use. 
...
of rectal cancer patients. Further examples include maximally selected statistics for 
...
H_0: D(\Y | \X) = D(\Y)
\end{eqnarray*}
against shift alternatives ($D$ describes the distribution of
a random variable). That is, departures from the null hypothesis
...
partitions of the observations into two samples.
...
$\Y$. (Because the margins are fixed, we only need to compute the first $k - 1$ columns
...
scheme in a randomized clinical trial or dependent sample designs (where
each observation forms a block) into account.
...
or binary response $\Y$, the distribution of the maximum of standard normal variables 
...
\end{eqnarray*}
With $\mathbf{v} = (\rho_{1,1}, \dots, \rho_{1,p})$ the lower triangular part of the correlation
matrix $\Rb$ can be written as $\mathbf{v} (1/\mathbf{v})^\top$ and it follows 
...
It should be noted that in a conditional setting, the number of cutpoints $p$
is treated as fix (this is obvious for nominal $\X$ at $k$ levels 
where $p = 2^{k-1}$ or when sample quantiles define the cutpoints). Also,
the approximation suggested by \cite{LausenSchumacher1996} using
an improved Bonferroni inequality conditions on $p$. In contrast,
the limiting Ornstein-Uhlenbeck process of $(T - \mu) / \sqrt{\text{diag}(\Sigma)}$ 
\citep{LausenSchumacher1992}
assumes $p \rightarrow \infty$ as $n \rightarrow \infty$.
Using the conditional asymptotic distribution leads to a
more powerful test compared to the approximations so far (see Figure~\ref{approx}).
\begin{figure}
\begin{center}
\caption{Approximations of the distribution of a maximally selected statistic
         for $n = 100$ observations (response rank-transformed) and $p = 20$ cutpoints. The exact distribution
         as approximated by $30.000$ random permutations of the data is
         shown as a solid line. The conditional asymptotic distribution suggested here
         is rather close. The approximations via the improved Bonferroni correction
         and two approximations of the distribution of the supremum of an Ornstein-Uhlenbeck
         process less accurate. \label{approx}}
...
legend("bottomright", lty = 1:5, 
       legend = c("Conditional Monte-Carlo", 
                  ...
                  "Improved Bonferroni, Worsley (1982)", 
                  "sup Ornstein-Uhlenbeck, Hansen (1999)",
                  "sup Ornstein-Uhlenbeck, Jennen (1985)"),  
       bty = "n")
@
\end{center}
\end{figure}
...
For multiple covariates, we simply look at all binary partitions induced by all 
...
statistics, which usually utilize an unconditional null distribution,
and to develop new maximally selected statistics 
for novel applications.

\paragraph{Maximally selected $\chi^2$ statistics.} 
The response variable is a factor at two levels $a$ and $b$, say, and $h = I(\Y_i = a)$
is a dummy coding of the levels.
The ordered univariate covariate $\X$ offers $p \le n - 1$ cutpoints $\xi_1, \dots, \xi_p$ 
leading to $g_j(\X_i) = I(X_i \le \xi_j)$. Thus,
\begin{eqnarray*}
...
$\Y_i = a$. Thus, $\T_j$ is one element of the $2 \times 2$ contingency
table for $I(\X \le \xi_j)$ and $I(\Y = a)$ and determines the 
complete table because the margins are fix. 
The statistic $(\T - \mu)^2 / \text{diag}(\Sigma)$
is equivalent to the usual $\chi^2$ statistic and our maximally selected 
statistic $\Tmax$ is a monotone transformation of the
maximally selected $\chi^2$ statistic proposed by 
\cite{MillerSiegmund1982}.
For nominal responses with $k > 2$ levels, the statistic 
...
The ordered response $\Y$ is measured at $k$ ordered levels 
$\Y \in \{l_1, \dots, l_k\}$, say. To each level 
a score $\gamma_j, j = 1, \dots, k$, is attached and the
influence function reads $h(\Y) = \gamma_{\Y}$. For arbitrary
scores, the corresponding test is known as linear-by-linear
association test \citep{agresti2002}, for the special case
$\gamma_j = j$ this corresponds to the Cochran-Armitage test.
Consequently, the statistic $\Tmax$  is equivalent to a maximally 
selected Cochran-Armitage statistic \citep{Betensky1999}, 
however, no restrictions to the choice of scores apply here.
...
Wilcoxon rank sum statistic. More general, $h$ can be any rank transformation
...
\paragraph{Three novel maximally selected statistics procedures.}
Since no restrictions to the measurement scales of covariate(s)
or response(s) apply, we are free to define new maximally selected
statistics for a wide range of problems. Here, concentrate on three 
...
for interactions. If there doesn't exist a good justification for
choosing a special rank transformation as influence function $h$, such
as ranks or normal scores, the identity transformation $h(\Y) = \Y$ 
can be applied. Each linear statistic $\T_j$ corresponds to
a two-sample permutation test against location alternatives. 
...
Finally, consider the situation of two ordered covariates. Instead
of looking at all splits in the first and second variable,
we only search for splits in all admissible interactions of 
both variables. In contrast to recursive partitioning
procedures selecting only one covariate for splitting in each node, 
such as CART \citep{CART1984} for example, we search for a
simultaneous split in both variables; such a procedure can solve
the famous XOR problem. \marginpar{ref?}
An application is given in the next section.
...
We attempt to identify high- and low-risk groups of
...
\Sexpr{round(teststat, 3)} (adjusted $p$-value smaller than $0.0001$; the
unadjusted $p$-value is about $2 \times \exp(\Sexpr{round(pnorm(teststat, lower.tail = FALSE, log = TRUE))})$. 
