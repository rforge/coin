...
differentiating them with respect to their combination of pathological T 
and N category (ordinal assessment of tumours and lymph nodes, respectively).
Patients were treated with a neo-adjuvant chemoradiotherapy regime starting before surgery,
possible prognostic factors for these patients are recently under 
investigation \citep{Roedel:2007}.
Survival times of $n = \Sexpr{length(tc)}$ patients from the preoperative arm 
of the CAO/\-ARO/\-AIO-94 trial \citep{Sauer2004} are under test;
$48$ patients with M category (assessment of metastates) 
equal to M1 were excluded from this analysis. 

We propose a new maximally selected statistic for a censored response
...
<<N0split, echo = FALSE>>=
n0 <- as.factor(preOP$nc == "pN0")
n0stat <- statistic(independence_test(Surv(time, event) ~ n0, data = preOP))
@

The TNM staging system \citep{Sobin+Wittekind:2002} for classifying the 
extent of cancer spread applies 
to patients who did not undergo chemoradiotherapy before surgical resection. 
The TNM stagings I, II and III are defined by fixed cutpoints in
the interaction of T and N (stage IV is based on the M category), therefore,
our procedure automatically includes TNM stagings as possible splits.
The T category distinguishes between stages I and II whereas the N category
is used to separate stages I/II (N0) and III ($\ge$ N1). 
Thus, the split between N1 and N2 identified by our procedure leads 
to a new prognostic group within TNM stage III. The log-rank statistic
comparing stages I/II and III is \Sexpr{round(n0stat, 3)}, so 
the split between N1 and N2 seems to be preferable to a split between N0 and N1.
%%\citep[details are currently under investigation][]{Roedel:2007}.
