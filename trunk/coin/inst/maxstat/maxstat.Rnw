...
These considerations about the asymptotic behaviour of $p$ also raise the
...
in categorical variable with large number of categories, e.g.,
$10$ categories with $10$ observations each leads to $n = 100$ but
$p = 2^{10 - 1} = 512$. But as each two-sample statistic is based on at least
...
For categorial responses, $h$ is typically a simple dummy
