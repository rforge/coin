...
in a second step we want to estimate the `best' cutpoint 
...
The complete correlation structure of the statistics to be maximized 
...
framework, closed-form expressions of the conditional expectation 
...
\cite{StrasserWeber1999} and can be found in \cite{Hothornetal2006} as well.
...
These values are then reused when computing $f_{j-1}$. 
...
%%We would like to thank Alan Genz for pointing us to \cite{Miwa2000}. Claus R\"odel
%%kindly gave us permission to illustrate the framework by an application
%%to the CAO/ARO/AIO-94 trial data.
