...
have to make sure that there is some association between response and covariate 
and
...
in a second step we want to estimate the `best' cutpoint describing this relationship. 
...
A linear-time algorithm for the evaluation of this asymptotic distribution
for potentially large numbers of cutpoints under test is proposed which 
significantly improves upon approximations for the asymptotic distribution
currently in use. 
Since no assumptions on the scale level of neither the covariate to be dichotomized nor
the response variable are necessary, the general concept can be used to implement
already published procedures and to extend the methodology to new areas of 
application. As an application of the proposed framework we construct and
apply a maximally selected log-rank statistic for an unordered bivariate covariate,
namely T and N-category of rectal cancer patients.
...
measurements only. Here,
\begin{eqnarray*}
g_j(\X_i) \quad = \quad \indic(\X_i \in A_j) \qquad (i = 1, \dots n)
\end{eqnarray*}
denotes the indicator function partitioning the observations 1, \dots, n 
into two groups. In case of ordered $\X$, each partition is associated with
a cutpoint. Only binary partitions satisfying the sample size 
...
Obtaining valid estimates of the mean and covariance of $\T$ 
requires a model to operate under and can be tedious. Alternatively,
we can apply the permutation test framework established
by \cite{StrasserWeber1999} by conditioning on all possible  
permutations $S$ of the responses $\Y_1, \dots, \Y_n$. Within this 
framework, closed-form expression of the conditional expectation 
$\mu = \E(\T | S) \in \R^{pq \times 1}$ and covariance
$\Sigma = \V(\T | S) \in \R^{pq \times pq}$ of $\T$ under $H_0$ given
all permutations $\sigma \in S$ of the responses are given by
\cite{StrasserWeber1999} and in \cite{Hothornetal2006}.
...
from the null hypothesis we
are interested in determining the partition with largest standardized statistic:
the best separating partition is $A_{j^\star}$ where
the absolute value of the standardized statistic $\T_{j^\star}$ equals $\Tmax$.
%%\begin{eqnarray*}
...
\citep[Theorem 3 in][]{StrasserWeber1999}.
...
\phi\left(\z^\top \Rb^{-1}\z \right) d\z = 1 - \frac{1}{\sqrt{|\Rb| (2\pi)^p}}
...
of $f_j(z)$ for a grid of $z$ values. For computing $f_{j-1}$, 
...
variables are given in the sequel \citep[further details are given by][]{Hothornetal2006}, 
...
dummy coding of the $k$ factor levels. 
It is sufficient to code for the first $k - 1$ levels only. 
...
When a numeric response is censored, log-rank transformations or
...
This flexible framework can know be utilized to implement already 
the conditional versions of published test statistics and to develop 
maximally selected statistics 
for new applications. For binary responses and ordered covariates,
...
corresponds to maximally selected McNemar's statistics
...
differentiate between two risk groups and develop a new maximally selected statistic
for a censored response and two unordered covariates.
...
From the potential $\Sexpr{2^(nlevels(preOP$tn)-1)}$ partitions,
...
\Sexpr{round(teststat, 3)} ($p$-value smaller than $0.001$). 
The partition chosen by the algorithm identifies all patients with
N category N2 or N3 as being under high risk. As an exception,
a single patient %Z% One should probably add some remark about this...
with ypT4 and N1 is assigned to the high risk group as well,
wheather or not this decision is sensible cannot be judged based on one observations. 
Figure~\ref{fig:maxstat} 
depicts Kaplan-Meier estimates of the survival times in the two risk groups.
...
distribution of the maximum statistic is known as well and can be evaluated efficiently
in the most important situations by numerical integration techniques. We thereby extend
the area of application of maximally selected statistics to problems with
a potentially large number of possible cutpoints.
...
\textsf{R} \citep{PKG:R} in the add-on package \Rpackage{coin} 
\citep{Hothornetal2006,PKG:coin}. The distribution
...
%%We would like to thank Alan Genz for pointing us to \cite{Miwa2000}.
