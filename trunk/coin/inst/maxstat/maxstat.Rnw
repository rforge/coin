...
maximally selected statistics is given and a new algorithm for the efficient 
evaluation of this potenially high-dimensional normal distribution is proposed.
...
\section{Approximations of the Reference Distribution}

The exact conditional distribition of $\Tmax$ under the conditions of $H_0$
is tracktable only under special conditions \citep{Boulesteix2006a,Boulesteix2006b}, especially
for small numbers of observations and a limited number of potential cutpoint.
In the general case, the 
exact conditional distribution of $\Tmax$ under $H_0$ can be approximated by 
Monte Carlo methods: Randomly draw a number of permutations of $1, \dots, n$, 
evaluate the test statistic $\Tmax$ for shuffled responses $\Y$ and compute the
proportion of random permutations where $\Tmax$ exceeds the test statistic for
the permutation $1, \dots, n$.
...
Thus, we have to evaluate $\Prob(\Tmax > c) = \Prob(\max(|Z_1|, ..., |Z_{pq}| > c)$ 
for standard normal random variables $Z_1, \dots, Z_{pq}$ with correlation matrix 
$\Rb = \text{cor}(\Sigma)$ and some $c > 0$. In fact,
...
dimensions ($pq > 50$, say). It should be noted that both approximations ignore the 
...
provides us with an upper bound for $\Prob(\Tmax > c)$ as $n \rightarrow \infty$ (see Appendix).

However, for the most important case of statistics maximally selected
over cutpoints induced by an ordered covariate $\X$, the probability
$\Prob(\Tmax > c)$ can be evaluated numerically by an algorithm with
computing time being linear in the number of cutpoints considered. 

Let $A_j = (-\infty, \xi_j]$ with $\xi_j < \xi_k$ for $1 \le j < k \le p$ 
denote the partitioning sets and let $q = 1$ (i.e., numeric or binary response variable). 
...
matrix $\Rb$ can be written as $\v (1/\v)^\top$ and it follows that the
...
\end{eqnarray*}
Because of the special band structure of $\Rb^{-1}$ the quadratic form $\z^\top \Rb^{-1}\z$ simplifies to
\begin{eqnarray*}
...
these values can be reused. Using these $p$-values leads to a much
